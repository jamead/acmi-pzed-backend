...
void reg_settings(void *msg) {
    ...
    switch(addr) {
        ...
        case SOFT_TRIG_MSG:
        	//xil_printf("Sending Soft Trigger from PV...\r\n");
        	//soft_trig_artix();
        	break;


        case EVR_EVENT_NO_MSG:
        	xil_printf("Setting EVR trigger number: %d\r\n",data.u);
        	Xil_Out32(XPAR_M_AXI_BASEADDR + EVR_FE_TRIGNUM_REG, data.u);
        	break;


        case EVR_EVENT_DLY_MSG:
         	xil_printf("Setting EVR trigger delay: %d\r\n",data.u);
         	Xil_Out32(XPAR_M_AXI_BASEADDR + EVR_FE_TRIGDLY_REG, data.u);
         	break;
